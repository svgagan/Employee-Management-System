...
<% Registration employee = (Registration)request.getAttribute("employee"); Payment pay = (Payment)request.getAttribute("payment"); %>
	...
	<table border="1" width="50%" height="100px">
		...
		<tbody>
			...
			<tr><td>Salary/annum</td><td><input type="text" disabled name="salary" value="<%=pay.getSalary()%>"/></td></tr>
			<tr><td>Bonus/month</td><td><input type="text" name="bonus" value="<%=pay.getBonus()%>"/></td></tr>
			<tr><td>Total/month</td><td><input type="text" name="total" disabled value="<%=pay.getTotal()%>"/></td></tr>
			<tr><td colspan="2" align="center"><input type="submit" value="Update Bonus"></td></tr>	
		</tbody>
	</table>
	</form>
<%}%></center>
